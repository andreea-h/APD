...
class ColorGraphTask extends RecursiveTask<Void> {
    int step;
    int[] colors;

    public ColorGraphTask(int[] colors, int step) {
        this.step = step;
        this.colors = colors;
    }
    ...
    protected Void compute()  {
        ...
        List<ColorGraphTask> tasks = new ArrayList<>();
        for (int i = 0; i < Main.COLORS; i++) {
            int[] newColors = colors.clone();
            newColors[step] = i;
            if (task2.Main.verifyColors(newColors, step)) {
                ColorGraphTask t = new ColorGraphTask(newColors, step + 1);
                tasks.add(t);
                t.fork();
            }
            for (ColorGraphTask task : tasks) {
                task.join();
            }
        }
        return null;
...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       // colorGraph(colors, 0);
        ...
        fjp.invoke(new ColorGraphTask(colors, 0));
        fjp.shutdown();
    }
}
