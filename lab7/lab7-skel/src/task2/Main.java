...
class ColorGraphTask implements Runnable {
    int step;
    int[] colors;

    ExecutorService tpe;

    public ColorGraphTask(int[] colors, ExecutorService tpe, int step) {
        this.tpe = tpe;
        this.step = step;
        this.colors = colors;
    }
    ...
    public void run() {
        ...
        for (int i = 0; i < Main.COLORS; i++) {
            ...
            if (Main.verifyColors(newColors, step)) {
                tpe.submit(new ColorGraphTask(newColors, tpe, step + 1));
            }
        }
    }
}
...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       // System.out.println("cu workeri:");
        ExecutorService tpe = Executors.newFixedThreadPool(4);
        tpe.submit(new ColorGraphTask(colors, tpe, 0));
    }
}
