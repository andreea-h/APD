...
int leader_chosing(int rank, int nProcesses) { //algoritmul heartbeat
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		...
		for (i = 0; i < num_neigh; i++) {
			MPI_Send(&leader, 1, MPI_INT, neigh[i], rank, MPI_COMM_WORLD);
		}
		for (i = 0; i < num_neigh; i++) {
			//primeste mesaj de la vecin
			int recv;
			MPI_Recv(&recv, 1, MPI_INT, neigh[i], MPI_ANY_SOURCE, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			//verifica si actualizeaza leader-ul
			if (recv > leader) {
				leader = recv;
			}
		}
