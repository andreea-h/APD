...
int main(int argc, char * argv[]) {
	...
	if (rank == MASTER) { // This code is run by a single process
		...
		*/
		...
		for (int i = 0; i < polynomialSize; i++) {
			...
			*/
			if (i != 0) {
				float coef = a[i];
				MPI_Send(&coef, 1, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
			}
		}
		...
		float coef_0 = a[0];
		MPI_Send(&coef_0, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD);
		...
		*/
		...
		//primeste coeficientul
		float recv_coef;
        MPI_Recv(&recv_coef, 1, MPI_FLOAT, MASTER, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        
        //primeste suma partiala
        float last_sum;
        MPI_Recv(&last_sum, 1, MPI_FLOAT, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        
        //primeste valoarea lui x
        MPI_Recv(&x, 1, MPI_INT, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        
        //calculeaza valoarea pt c * x^r si aduna la suma
        val = recv_coef * pow(x, rank);
        sum = last_sum + val;
