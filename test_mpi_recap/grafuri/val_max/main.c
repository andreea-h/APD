...
//HEARBEAT- determinarea valorii maxime detinuta de nodurile dintr-un graf conex
//fiecare nod din graf are o valoare si un vector de vecini
//se cere sa se afle valoarea maxima si sa se propage aceasta valoare la toate nodurile
//valoarea maxima este propagata catre toate nodurile
...
static int *neigh;
static int value; //valoarea pe care o retine fiecare nod din graf (adica fiecare proces)
...
void read_neighbours(int rank) {
	...
	fscanf(fp, "%d", &value); //valoarea pe care o are fiecare proces
...
//folosesc algoritmul hearbeat pentru a determina valoarea maxima 
//retinuta de nodurile grafului
int leader_chosing(int rank, int nProcesses) { //algoritmul heartbeat
	int local_max_val = value; //valoarea maxima este initializata cu valoarea detinuta de procesul curent
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		...
		for (i = 0; i < num_neigh; i++) {
			MPI_Send(&local_max_val, 1, MPI_INT, neigh[i], rank, MPI_COMM_WORLD);

			//primeste mesaj de la vecin
			int recv;
			MPI_Recv(&recv, 1, MPI_INT, neigh[i], MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			//verifica si actualizeaza leader-ul local
			if (recv > local_max_val) {
				local_max_val = recv;
			}
		}
	}
	...
	printf("%i/%i: max_value is %i\n", rank, nProcesses, local_max_val);
...
int main(int argc, char * argv[]) {
	...
	int max_value = leader_chosing(rank, nProcesses);

	//printf("[result] Valorea maxima este: %d\n", max_value);
	/*MPI_Barrier(MPI_COMM_WORLD);
